...
var anch = function anch() {
  ...
  headerMenu.addEventListener('click', function (event) {
    ...
    if (target.classList.contains('company')) {
      ...
      document.querySelector('.burger-overlay').classList.toggle('burger-overlay-open');
...
var burgerMenu = function burgerMenu() {
  ...
  btn.addEventListener('click', function () {
    ...
    overlay.classList.toggle('burger-overlay-open');
    var padding = innerWidth - document.body.offsetWidth + 'px';
...
var cart = function cart(data) {
  var local = {};
  var cartCount = document.querySelector('.total-result');

  var calc = function calc() {
    var sum = 0;
    Object.keys(local).forEach(function (item) {
      sum += local[item].price * local[item].count;
    });
    var count = +cartCount.textContent;

    if (count < sum) {
      var k = Math.ceil((sum - count) / 20);
    ...
    } else cartCount.textContent = sum;
  };

  var render = function render() {
    document.querySelector('.cart-inner').textContent = '';

    if (!Object.keys(local).length) {
      document.querySelector('.header-cart-count').style.display = 'none';
    } else document.querySelector('.header-cart-count').textContent = Object.keys(local).length;

    Object.keys(local).forEach(function (item) {
      var newItem = document.createElement('div');
      newItem.classList.add('cart-item');
      newItem.setAttribute('data-id', item);
      newItem.setAttribute('data-category', local[item].category);
      var newItemId = item;
      var newItemCategory = local[item].category;
      var newItemIndex;
      data[newItemCategory].forEach(function (item, index) {
        if (item.id === +newItemId) {
          newItemIndex = index;
        }
      });
      newItem.innerHTML = "\n      <a href=\"#\" class=\"cart-item-link\">\n      <img src=\"images/".concat(newItemCategory, "/").concat(data[newItemCategory][newItemIndex].image, "\" alt=\"\" class=\"cart-item-img\">\n    </a>\n    <div class=\"cart-item-info\">").concat(data[newItemCategory][newItemIndex].name, "</div>\n    <div class=\"cart-count-box\">\n      <div class=\"cart-count-btn cart-count-minus\">\n        <img src=\"images/minus.svg\" alt=\"\" class=\"cart-count-minus\">\n      </div>\n      <output class=\"cart-item-count\">").concat(local[item].count, "</output>\n      <div class=\"cart-count-btn cart-count-plus\">\n        <img src=\"images/cart-plus.svg\" alt=\"\" class=\"cart--count-plus\">\n      </div>\n    </div>\n    <div class=\"cart-item-price-box\">\n      <output class=\"cart-item-full-price\">").concat(local[item].count * local[item].price, " BYN</output>\n      <div class=\"cart-item-price\">").concat(local[item].price, " BYN/\u0448\u0442.</div>\n    </div>\n    <div class=\"cart-item-delete-btn\">\n      <svg width=\"38\" height=\"38\" viewBox=\"0 0 38 38\" fill=\"none\" xmlns=\"http://www.w3.org/2000/svg\">\n        <circle cx=\"19\" cy=\"19\" r=\"18.5\" stroke=\"white\" stroke-opacity=\"0.5\"/>\n        <path d=\"M14 14L19.25 19.25M24.5 24.5L19.25 19.25M19.25 19.25L24.5 14M19.25 19.25L14 24.5\" stroke=\"white\" stroke-opacity=\"0.5\"/>\n        </svg>\n    </div>\n      ");
      document.querySelector('.cart-inner').append(newItem);
    });
    calc();
  };

  var getLocal = function getLocal() {
    if (localStorage.getItem('smeg')) {
      local = JSON.parse(localStorage.getItem('smeg'));
      render();
    }
  };
  ...
  var animate = function animate(item) {
    var target = item.closest('.cart-item');
    target.querySelector('.cart-item-count').textContent = local[target.getAttribute('data-id')].count;
    var itemCurrent = target.closest('.cart-item').querySelector('.cart-item-full-price');
    var count = parseInt(itemCurrent.textContent);
    var sum = local[target.closest('.cart-item').getAttribute('data-id')].price * local[target.closest('.cart-item').getAttribute('data-id')].count;
    ...
    if (count < sum) {
      ...
      var interval = setInterval(function () {
        ...
        if (count > sum) {
          itemCurrent.textContent = sum + ' BYN';
          clearInterval(interval);
          return;
        }

        itemCurrent.textContent = count + ' BYN';
      }, 15);
    } else if (count > sum) {
      var _k2 = Math.ceil((count - sum) / 20);
    ...
    } else itemCurrent.textContent = sum + ' BYN';

    calc();
  };

  var deleteItemAnimate = function deleteItemAnimate(item) {
    var target = item.closest('.cart-item');
  ...
  document.querySelector('.cart-inner').addEventListener('click', function (e) {
    var target = e.target;
    var item = target.closest('.cart-item');

    if (target.closest('.cart-count-minus')) {
      var itemId = item.getAttribute('data-id');

      if (local[itemId].count > 1) {
        local[itemId].count--;
        localStorage.setItem('smeg', JSON.stringify(local));
        animate(target);
      }
    } else if (target.closest('.cart-count-plus')) {
      var _itemId = item.getAttribute('data-id');

      local[_itemId].count++;
      localStorage.setItem('smeg', JSON.stringify(local));
      animate(target);
    } else if (target.closest('.cart-item-delete-btn')) {
      var _itemId2 = item.getAttribute('data-id');

      delete local[_itemId2];
      localStorage.setItem('smeg', JSON.stringify(local));
      deleteItemAnimate(target);
    }
  });
};
...
var data = {
  popular: [{
    id: 1,
    name: 'Чайник электрический KLF03RDEU',
    price: 300,
    image: 'pop1.png'
  }, {
    id: 2,
    name: 'Блендер BLF01PEU',
    price: 365,
    image: 'pop2.png'
  }, {
    id: 3,
    name: 'Капельная кофеварка DCF02PBEU',
    price: 810,
    image: 'pop3.png'
  }, {
    id: 4,
    name: 'Тостер TSF01PBEU',
    price: 360,
    image: 'pop4.png'
  }, {
    id: 5,
    name: 'Холодильник FAB38LBL',
    price: 8000,
    image: 'pop5.png'
  }, {
    id: 6,
    name: 'Кофеварка рожковая ECFCREU',
    price: 1000,
    image: 'pop6.png'
  }, {
    id: 7,
    name: 'Миксер планетарный SMF01PBEU',
    price: 1500,
    image: 'pop7.png'
  }, {
    id: 8,
    name: 'Погружной блендер HBF01CREU',
    price: 280,
    image: 'pop8.png'
  }, {
    id: 1,
  ...
  teapots: [{
    id: 31,
    name: 'Чайник электрический KLF03WHEU цвет Белый, Объем: 1,7 л; 2,2-2,4 кВт',
    price: 500,
    image: '1.png'
  }, {
    id: 32,
    name: 'Чайник электрический KLF03BLEU цвет Черный, Объем: 1,7 л; 2,2-2,4 кВт',
    price: 500,
    image: '2.png'
  }, {
    id: 33,
    name: 'Чайник с регулируемой температурой KLF04CREU цвет кремовый, Объем: 1,7 л; 2,2-2,4 кВт',
    price: 650,
    image: '3.png'
  }, {
    id: 34,
    name: 'Чайник электрический KLF03RDEU цвет Красный, Объем: 1,7 л; 2,2-2,4 кВт',
    price: 500,
    image: '4.png'
  }, {
    id: 35,
    name: 'Чайник электрический KLF03GOEU цвет золото, Объем: 1,7 л; 2,2-2,4 кВт',
    price: 650,
    image: '5.png'
  }, {
    id: 36,
    name: 'Чайник электрический KLF03RGEU цвет: розовое золото, Объем: 1,7 л; 2,2-2,4 кВт',
    price: 650,
    image: '6.png'
  }, {
    id: 37,
    name: 'Чайник электрический KLF03DGEU Коллекция: D&G, Объем: 1,7 л; 2,2-2,4 кВт',
    price: 1500,
    image: '7.png'
  }, {
    id: 38,
    name: 'Чайник электрический KLF03PBEU Цвет: голубой, Объем: 1,7 л; 2,2-2,4 кВт',
    price: 500,
    image: '8.png'
  }, {
    id: 39,
    name: 'Чайник электрический KLF03PBEU Цвет: розовый, Объем: 1,7 л; 2,2-2,4 кВт',
    price: 500,
    image: '9.png'
  }]
};
...
var form = function form() {
  var form = document.querySelector('#footer-form'),
      status = document.querySelector('.footer-form-status'),
      server = form.getAttribute('data-server');
  var message = {
    loading: 'Загрузака...',
  ...
  form.addEventListener('submit', function (e) {
    ...
    postData(server, formData).then(function (res) {
      form.reset();
      status.textContent = message.succes;
    }).catch(function () {
      return status.textContent = message.failure;
    }).finally(function () {
      setTimeout(function () {
        status.textContent = '';
      }, 3000);
...
var formCart = function formCart() {
  var inner = document.querySelector('.popular-arrow-wrapper');
  var cartIndex = document.querySelector('.header-cart-count');
  var cart = {};
  var elems = document.querySelectorAll('.popular-item-wrapper');
  var defaultLocal = {
    "31": {
      "price": "500",
      "count": 1,
  ...
  var render = function render() {
    elems = document.querySelectorAll('.popular-item-wrapper');
    elems.forEach(function (elem) {
      if (cart.hasOwnProperty(elem.getAttribute('data-id'))) {
        elem.querySelector('.popular-item-btn').classList.add('popular-item-btn-done');
      } else {
        elem.querySelector('.popular-item-btn').classList.remove('popular-item-btn-done');
      }
    });

    if (!Object.keys(cart).length) {
      cartIndex.style.display = 'none';
    } else {
      cartIndex.style.display = 'block';
      cartIndex.textContent = Object.keys(cart).length;
    }
  };

  var getLocal = function getLocal() {
    if (localStorage.getItem('smeg')) {
      cart = JSON.parse(localStorage.getItem('smeg'));
      render();
    } else {
      localStorage.setItem('smeg', JSON.stringify(defaultLocal));
      getLocal();
    }
  };
  ...
  inner.addEventListener('click', function (e) {
    ...
    if (target && target.classList.contains('popular-item-btn')) {
      e.preventDefault();
      var targetId = target.closest('.popular-item-wrapper').getAttribute('data-id');
      var category = target.closest('.popular-item-wrapper').getAttribute('data-category');
      var price = target.closest('.popular-item-wrapper').querySelector('.popular-price-count').textContent;
      ...
      localStorage.setItem('smeg', JSON.stringify(cart));
      render();
    }
  });
};

/* harmony default export */ __webpack_exports__["default"] = (formCart);
...
var get = function get(data) {
  data.forEach(function (item) {
    var newItem = document.createElement('div');
    newItem.classList.add('swiper-slide', 'popular-item-wrapper');
    newItem.setAttribute('data-id', item.id);
    newItem.setAttribute('data-category', 'popular');
    newItem.innerHTML = "\n    <div class=\"popular-item\">\n    <div class=\"popular-item-btn\"></div>\n    <a href=\"#\" class=\"popular-item-link\">\n      <img src=\"images/popular/".concat(item.image, "\" alt=\"\" class=\"popular-item-img\">\n    </a>\n    <div class=\"popular-item-name\">").concat(item.name, "</div>\n    <div class=\"popular-item-price\"><span class=\"popular-price-count\">").concat(item.price, "</span> BYN</div>\n    </div>\n    ");
    document.querySelector('.popular-slider-inner').append(newItem);
  });
};
...
var getTeapots = function getTeapots(data) {
  data.forEach(function (item) {
    var newItem = document.createElement('div');
    newItem.classList.add('teapot-item', 'popular-item-wrapper');
    newItem.setAttribute('data-id', item.id);
    newItem.setAttribute('data-category', 'teapots');
    newItem.innerHTML = "\n    <div class=\"popular-item teapot-item-inner\">\n    <div class=\"popular-item-btn\"></div>\n    <a href=\"#\" class=\"popular-item-link teapot-item-link\">\n      <img src=\"images/teapots/".concat(item.image, "\" alt=\"\" class=\"popular-item-img\">\n    </a>\n    <div class=\"popular-item-name teapot-item-info\">").concat(item.name, "</div>\n    <div class=\"popular-item-price\"><span class=\"popular-price-count\">").concat(item.price, "</span> BYN</div>\n    </div>\n    ");
    document.querySelector('.teapot-items').append(newItem);
  });
};

/* harmony default export */ __webpack_exports__["default"] = (getTeapots);
...
/* harmony import */ var _burger__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./burger */ "./app/js/burger.js");
/* harmony import */ var swiper__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! swiper */ "./node_modules/swiper/swiper.esm.js");
/* harmony import */ var _get__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./get */ "./app/js/get.js");
/* harmony import */ var _data__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./data */ "./app/js/data.js");
/* harmony import */ var _formCart__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ./formCart */ "./app/js/formCart.js");
/* harmony import */ var _anchors__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ./anchors */ "./app/js/anchors.js");
/* harmony import */ var _form__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! ./form */ "./app/js/form.js");
/* harmony import */ var _getTeapots__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(/*! ./getTeapots */ "./app/js/getTeapots.js");
/* harmony import */ var _cart__WEBPACK_IMPORTED_MODULE_9__ = __webpack_require__(/*! ./cart */ "./app/js/cart.js");
...
var cartCounts = function cartCounts() {
  var countItem = document.querySelector('.header-cart-count');

  if (localStorage.getItem('smeg')) {
    var localLength = Object.keys(JSON.parse(localStorage.getItem('smeg'))).length;

    if (localLength === 0) {
      countItem.style.display = 'none';
    } else countItem.textContent = localLength;
  }
};
...
if (dataTitle === 'main') {
  ...
  var slider = new swiper__WEBPACK_IMPORTED_MODULE_2__["default"]('.slider', {
    slidesPerView: 1,
    loop: true,
    autoplay: {
      delay: 3000,
  ...
  Object(_get__WEBPACK_IMPORTED_MODULE_3__["default"])(_data__WEBPACK_IMPORTED_MODULE_4__["default"].popular);
  var popularSlider = new swiper__WEBPACK_IMPORTED_MODULE_2__["default"]('.popular-slider', {
    slidesPerView: 1,
    slidesPerColumn: 2,
  ...
  Object(_formCart__WEBPACK_IMPORTED_MODULE_5__["default"])();
}

;

if (dataTitle === 'teapots') {
  Object(_getTeapots__WEBPACK_IMPORTED_MODULE_8__["default"])(_data__WEBPACK_IMPORTED_MODULE_4__["default"].teapots);
  Object(_formCart__WEBPACK_IMPORTED_MODULE_5__["default"])();
}
...
if (dataTitle === 'cart') {
  Object(_cart__WEBPACK_IMPORTED_MODULE_9__["default"])(_data__WEBPACK_IMPORTED_MODULE_4__["default"]);
}

;
Object(_burger__WEBPACK_IMPORTED_MODULE_1__["default"])();
Object(_anchors__WEBPACK_IMPORTED_MODULE_6__["default"])();
